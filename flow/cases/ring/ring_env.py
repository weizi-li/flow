...
ADDITIONAL_ENV_PARAMS = {
    ...
    'max_decel': 3,
    # desired velocity for all vehicles in the simulation, in m/s
    'target_velocity': 10,
...
class AccelEnv(Env):
    ...
    * target_velocity: desired velocity for all vehicles in the simulation, in m/s
    * sort_vehicles: specifies whether vehicles are to be sorted by position
    ...
    States
        The state consists of the velocities and absolute positions of all
        vehicles in the simulation. This assumes a constant number of vehicles.

    Actions
        Actions are a list of acceleration for each RL vehicle, bounded by the
        maximum accelerations and decelerations specified in EnvParams.

    Rewards
        The reward function is the two-norm of the distance of the speed of the
        vehicles in the simulation from the "target_velocity" term. For a
        description of the reward, see: flow.core.rewards.desired_speed

    Termination
        A rollout is terminated if the time horizon is reached or if two
        vehicles collide into one another (reflected by the teleportation behavior
        from SUMO).
    ...
    def __init__(self, env_params, sim_params, simulation, sim_interface='traci'):
        ...
        super().__init__(env_params, sim_params, simulation, sim_interface)
    ...
    def additional_command(self):
        ...
        """
        # specify observed vehicles, since in this enviroment, we observe everything
    ...
    @property
    def sorted_ids(self):
        """Sort the vehicle ids of vehicles in the simulation by position.
