...
class Env(gym.Env):
    ...
    Provides the interface for interacting with various aspects of the
    simulation. Using this class, you can start a simulation instance,
    configure a simulation, specify controllers, perform simulation
    ...
    * compute_reward
    ...
    initial_veh_config : flow.core.params.InitialVehConfig
        see flow/core/params.py
    simulation : flow.simulation.Simulation
        see flow/simulation/base.py
    sim_interface : str
        the simulator interface used, one of {'traci', 'aimsun'}
    ...
    initial_veh_state : dict
        initial state information for all vehicles. The simulation is always
    ...
    initial_veh_ids : list of str
        ...
        corresponds to `self.initial_veh_state.keys()`.
    ...
    def __init__(self,
        ...
        simulation : flow.simulation.Simulation
            see flow/simulation/base.py
        sim_interface : str
            the simulator interface used, one of {'traci', 'aimsun'}. Default is 'traci'.
        ...
        # get more simulation parameters
        self.net_params = self.simulation.net_params
        self.initial_veh_config = self.simulation.initial_veh_config
        ...
        # store the initial vehicles and ids
        self.initial_vehicles = deepcopy(self.k.vehicle)
        self.initial_veh_ids = deepcopy(self.simulation.vehicles.ids)
    ...
    def setup_initial_state(self):
        ...
        if self.initial_veh_config.shuffle:
            random.shuffle(self.initial_ids)

        # generate starting position for vehicles in the simulation
        start_pos, start_lanes = self.k.network.generate_starting_positions(
            initial_veh_config=self.initial_veh_config,
    ...
    def reset(self):
        ...
        if self.sim_params.restart_instance or \
                (self.step_counter > 2e6 and self.sim_interface != 'aimsun'):
            self.step_counter = 0
        ...
        elif self.initial_veh_config.shuffle:
            self.setup_initial_state()
