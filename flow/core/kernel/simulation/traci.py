...
from flow.core.kernel.simulation import BaseKernelSimulation
...
class TraCIKernelSimulation(BaseKernelSimulation):
    """Base simulation kernel for SUMO-based simulations.

    This class initializes a new simulation, including the positions of nodes,
    properties of the edges and junctions connecting these nodes, properties
    of vehicles and traffic lights, and other features as well.
    """

    def __init__(self, master_kernel, sim_addl_params):
        """Instantiate a SUMO simulation kernel.
        ...
        """
        super(TraCIKernelSimulation, self).__init__(master_kernel, sim_addl_params)

        # directories for the simulation-specific files that will be generated by
        # the `generate_simulation` method
        ...
        # variables to be defined during simulation generation
    ...
    def generate_simulation(self, simulation):
        ...
        This class uses simulation-specific features to generate the necessary xml
        ...
        simulation : flow.core.sim_base
            an object containing relevant simulation-specific features such as the
            locations and properties of nodes and edges in the simulation.
        """
        # store the simulation object in the simulation variable
        self.simulation = simulation
        self.orig_name = simulation.orig_name
        self.name = simulation.name

        # names of the soon-to-be-generated xml and SUMO config files
        self.nodfn = '%s.nod.xml' % self.simulation.name
        self.edgfn = '%s.edg.xml' % self.simulation.name
        self.typfn = '%s.typ.xml' % self.simulation.name
        self.cfgfn = '%s.netccfg' % self.simulation.name
        self.netfn = '%s.net.xml' % self.simulation.name
        self.confn = '%s.con.xml' % self.simulation.name
        self.roufn = '%s.rou.xml' % self.simulation.name
        self.addfn = '%s.add.xml' % self.simulation.name
        self.sumfn = '%s.sumo.cfg' % self.simulation.name
        self.guifn = '%s.gui.cfg' % self.simulation.name

        # can only provide one of OSM path or template path to the simulation
        assert self.simulation.net_params.template is None \
            or self.simulation.net_params.osm_path is None

        # create the simulation configuration files
        if self.simulation.net_params.template is not None:
            self._edges, self._connections = self.generate_net_from_template(
                self.simulation.net_params)
        elif self.simulation.net_params.osm_path is not None:
            self._edges, self._connections = self.generate_net_from_osm(
                self.simulation.net_params)
        else:
            # combine all connections into a list
            if simulation.connections is not None:
                if isinstance(simulation.connections, list):
                    connections = simulation.connections
                else:
                    connections = []
                    for key in simulation.connections.keys():
                        connections.extend(simulation.connections[key])
            else:
                connections = None
            ...
            self._edges, self._connections = self.generate_net(
                ...
                self.simulation.edge_types,
                connections
            )

        # list of edges and junctions
        self._edge_list = [
    ...
    def close(self):
        """Close the simulation class.

        Deletes the xml files that were created by the simulation class. This
        ...
        """
        if self.simulation.net_params.template is None:
            try:
    ...
    def get_x(self, edge, position):
        ...
        if edge[0] == ':':
            try:
                return self.junction_starts_dict[edge] + position
            except KeyError:
                # in case several junctions (internal links) are being generalized for
    ...
    def generate_net(self,
        ...
        * connections (optional): describes how incoming and outgoing edge/lane
        ...
        # - output net file
        # - processing parameters for no junctions (internal links) and no turnarounds
    ...
    def generate_cfg(self, net_params, traffic_lights, routes):
        ...
        for params in self.simulation.vehicles.edge_types:
            type_params_str = {
        ...
        if self.simulation.net_params.inflows is not None:
            total_inflows = self.simulation.net_params.inflows.get()
