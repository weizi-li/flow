...
from flow.core.params import DEFAULT_VEH_LENGTH


class BaseKernelSimulation(object):
    """Base simulation kernel.

    This kernel subclass is responsible for generating any simulation-specific
    components needed to run a simulation. This may include network
    configuration files that support the generation of certain traffic
    networks in a simulator (e.g., SUMO), or may be as simple as passing simulation
    features from the simulation class (see flow/core/sim_base.py) and
    transferring them to the simulator kernel later on.

    In addition to generating files for simulation initialization, the simulation
    kernel performs two auxiliary tasks:

    * State acquisition: The simulation kernel contains several methods that can
      be used to acquire state information on the properties of the simulation,
      e.g., the number of lanes on an edge, the length
      ...
      travel within the simulation, this can be done by calling the following
      command:

        >>> from flow.core.sim_base import Env
        >>> env = Env(...)
        >>> max_speed = env.k.simulation.max_speed()
    ...
    * Methods for generating initial vehicle positions: Initial vehicle
      positions are generated by the abstract simulation kernel, and may be
      overridden by individual simulation cases, if the spacing in ``initial_veh_config``
      is set to random. Default initial positions include uniform starting positions
      (where all vehicles are equally spacing) or random starting positions (limited by some min_gap).
    """

    def __init__(self, master_kernel, sim_addl_params):
        ...
        self.sim_addl_params = sim_addl_params
        self.kernel_api = None

        # These variable need to be filled in by the generate_simulation method.
    ...
    def generate_simulation(self, simulation):
        """Generate the necessary prerequisites for a simulation.

        Parameters
        ----------
        simulation : flow.core.sim_base
            an object containing relevant simulation-specific features.
        """
    ...
    def update(self, reset):
        """Update the road network with current state information.
    ...
    def max_speed(self):
        """Return the maximum achievable speed on any edge in the road network."""
        raise NotImplementedError
    ...
    def get_edge_list(self):
        """Return the names of all edges in the road network."""
        raise NotImplementedError

    def get_junction_list(self):
        """Return the names of all junctions in the road network."""
        raise NotImplementedError
    ...
    def get_edge(self, x):  # TODO: maybe remove
        ...
        ----------
        x : float
            absolute position in road network
    ...
    def next_edge(self, edge, lane):
        ...
        These edges may also be junctions. Returns an empty
    ...
    def generate_starting_positions(self, initial_veh_config, num_vehicles=None):
        """Generate starting positions for vehicles in the simulation.
        ...
        num_vehicles : int, optional
            number of vehicles to be placed on the road network. If no value is
        ...
        """
        num_vehicles = num_vehicles or self.simulation.vehicles.num_vehicles

        if initial_veh_config.spacing == 'uniform':
            startpositions, startlanes = self.gen_even_start_pos(
                initial_veh_config, num_vehicles)
        elif initial_veh_config.spacing == 'random':
            startpositions, startlanes = self.gen_random_start_pos(
                initial_veh_config, num_vehicles)
        elif initial_veh_config.spacing == 'custom':
            startpositions, startlanes = self.gen_custom_start_pos(
                initial_veh_config, num_vehicles)
        else:
            raise FatalFlowError('"spacing" argument in initial_veh_config does '
                                 'not contain a valid option')

        return startpositions, startlanes

    def gen_even_start_pos(self, initial_veh_config, num_vehicles):
        """Generate uniformly spaced starting positions.

        If the perturbation term in initial_veh_config is set to some positive
        ...
        list of int
            list of start lanes
        """
        if isinstance(initial_veh_config.edges_distribution, dict):
            # check that the number of vehicle in edges_distribution matches
            # that of the vehicles class
            num_vehicles_e = sum(initial_veh_config.edges_distribution[k]
                                 for k in initial_veh_config.edges_distribution)
        ...
        (x0, min_gap, bunching, lanes_distr, available_length,
         available_edges, initial_veh_config) = \
            self._get_start_pos_util(initial_veh_config, num_vehicles)
        ...
        while car_count < num_vehicles:
            ...
            if flag and pos[1] < DEFAULT_VEH_LENGTH:
                pos0, pos1 = pos
                pos = (pos0, DEFAULT_VEH_LENGTH)
                x += DEFAULT_VEH_LENGTH
                increment -= (DEFAULT_VEH_LENGTH * self.num_lanes(pos0)) / \
            ...
            x = (x + increment + DEFAULT_VEH_LENGTH + min_gap) % self.length()

        # add a perturbation to each vehicle, while not letting the vehicle
        # leave its current edge
        if initial_veh_config.perturbation > 0:
            for i in range(num_vehicles):
                perturb = np.random.normal(0, initial_veh_config.perturbation)
    ...
    def gen_random_start_pos(self, initial_veh_config, num_vehicles):
        ...
        (x0, min_gap, bunching, lanes_distr, available_length,
         available_edges, initial_veh_config) = self._get_start_pos_util(
            initial_veh_config, num_vehicles)
        ...
        efs = min_gap + DEFAULT_VEH_LENGTH  # extra front space
        ...
        # to be added
        for i in range(num_vehicles):
            init_absolute_pos[i] += (DEFAULT_VEH_LENGTH + min_gap) * i
    ...
    def gen_custom_start_pos(self, initial_veh_config, num_vehicles):
        """Generate a user defined set of starting positions.

        This is called straight from the simulation class.
        ...
        list of int
            list of start lanes
        """
        return self.simulation.gen_custom_start_pos(
            cls=self,
            net_params=self.simulation.net_params,
            initial_veh_config=initial_veh_config,
            num_vehicles=num_vehicles,
        )

    def _get_start_pos_util(self, initial_veh_config, num_vehicles):
        """Prepare initial_veh_config data for starting position methods.

        Performs some pre-processing to the initial_veh_config and **kwargs terms,
        and returns the necessary values for all starting position generating
        functions.

        Parameters
        ----------
        initial_veh_config : InitialVehConfig type
            see flow/core/params.py
        num_vehicles : int
            number of vehicles to be placed on the network
        ...
        initial_veh_config : InitialVehConfig type
            modified version of the initial_veh_config parameter
        ...
        """
        min_gap = max(0, initial_veh_config.min_gap)

        bunching = initial_veh_config.bunching
        # check if requested bunching value is not valid (negative)
        if initial_veh_config.bunching < 0:
            logging.warning('"bunching" cannot be negative; setting to 0')
            initial_veh_config.bunching = 0

        # compute the lanes distribution (adjust of edge cases)
        if initial_veh_config.edges_distribution == 'all':
            max_lane = max(
                [self.num_lanes(edge_id) for edge_id in self.get_edge_list()])
        else:
            max_lane = max([
                self.num_lanes(edge_id)
                for edge_id in initial_veh_config.edges_distribution
            ])

        if initial_veh_config.lanes_distribution > max_lane:
            lanes_distribution = max_lane
        elif initial_veh_config.lanes_distribution < 1:
            logging.warning('"lanes_distribution" is too small; setting to 1')
            lanes_distribution = 1
        else:
            lanes_distribution = initial_veh_config.lanes_distribution
        ...
        if initial_veh_config.edges_distribution == 'all':
            distribution_length = \
                sum(self.edge_length(edge_id) *
                    ...
                    if self.edge_length(edge_id) > min_gap + DEFAULT_VEH_LENGTH)
        else:
            distribution_length = \
                sum(self.edge_length(edge_id) *
                    min(self.num_lanes(edge_id), lanes_distribution)
        ...
        if initial_veh_config.edges_distribution == 'all':
            available_edges = [
                ...
                if self.edge_length(edge) > min_gap + DEFAULT_VEH_LENGTH]
        else:
            available_edges = [
                edge for edge in initial_veh_config.edges_distribution
                if self.edge_length(edge) > min_gap + DEFAULT_VEH_LENGTH]

        available_length = \
            distribution_length - lanes_distribution * bunching - \
            num_vehicles * (min_gap + DEFAULT_VEH_LENGTH)

        if available_length < 0:
            raise FatalFlowError('There is not enough space to place all '
                                 'vehicles in the network.')

        return (initial_veh_config.x0, min_gap, bunching, lanes_distribution,
                available_length, available_edges, initial_veh_config)
